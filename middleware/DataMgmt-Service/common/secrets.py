...
Settings values like `DB_PASSWORD` or `AIRFLOW_PASSWORD` can be either a literal
value (local dev) or a secret reference resolved at startup against AWS Secrets
Manager, Azure Key Vault, or GCP Secret Manager. Same env var name across all
environments — deployment manifests just point at a different concrete source.
